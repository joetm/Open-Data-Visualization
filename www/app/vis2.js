...
var c3_graph = {
    ...
    open_agency_modal: function (agency_name) {
        ...
        this.set_agency_timeseries(agency_name);

        //console.log(this.graph_data);return;
        ...
        try { //there could be agencies with no data
            ...
            alert('Data for this agency is not available');
            console.log(err);
            return; //abort modal
        }
...
function array_extent(field, subject_array) {
    'use strict';
    var max_array = [];
    $.each(subject_array, function () {
        max_array.push(
            d3.sum(this, function (d) {
                return +d[field];
            })
        );
    });
...
function update_graph_bounds(projects) {
    'use strict';
    ...
    //build an agency array
    for (i = 0; i < s; i = i + 1) {
        //console.log(projects[i]);break;
        if (tmp_agencies[projects[i][fields.agency_name]] === undefined) {
            tmp_agencies[projects[i][fields.agency_name]] = [];
        }
        tmp_agencies[projects[i][fields.agency_name]].push(projects[i]);
    }//for: loop projects
    //console.log(tmp_agencies);return;


    //minimum lifecycle_cost_m scale
    graph.min.lifecycle_cost_m = 0;

    //maximum lifecycle_cost_m scale
    graph.max.lifecycle_cost_m = d3.max(array_extent(fields.lifecycle_cost_m, tmp_agencies), function (d) { return d; });
    //console.log('Max lifecycle cost: ' + graph.max.lifecycle_cost_m);return;


    //minimum planned_cost_m scale
    graph.min.planned_cost_m = 0;

    //maximum planned_cost_m scale
    graph.max.planned_cost_m = d3.max(array_extent(fields.planned_cost_m, tmp_agencies), function (d) { return d; });
    //console.log('Max planned cost: ' + graph.max.planned_cost_m); return;


    //minimum cost_variance_m scale
    graph.min.cost_variance_m = d3.min(array_extent(fields.cost_variance_m, tmp_agencies), function (d) { return d; });

    //maximum cost_variance_m scale
    graph.max.cost_variance_m = d3.max(array_extent(fields.cost_variance_m, tmp_agencies), function (d) { return d; });
    //console.log('graph.max.cost_variance_m: ', graph.max.cost_variance_m);return;


    //minimum schedule_variance_in_days scale
    graph.min.schedule_variance_in_days = d3.min(array_extent(fields.schedule_variance_in_days, tmp_agencies), function (d) { return d; });
    //console.log(graph.min.schedule_variance_in_days);return;

    //maximum schedule_variance_in_days scale
    graph.max.schedule_variance_in_days = d3.max(array_extent(fields.schedule_variance_in_days, tmp_agencies), function (d) { return d; });
    //console.log(graph.max.schedule_variance_in_days);return;


    //minimum schedule_variance scale
    graph.min.schedule_variance = d3.min(array_extent(fields.schedule_variance, tmp_agencies), function (d) { return d; });
    //console.log(graph.min.schedule_variance);return;

    //maximum schedule_variance scale
    graph.max.schedule_variance = d3.max(array_extent(fields.schedule_variance, tmp_agencies), function (d) { return d; });
...
$(document).ready(function () {
    ...
    $('#filters dd').click(function () {
        ...
        //update_graph_bounds(projects); //not needed (min and max values in data did not change)
